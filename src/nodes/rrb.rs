...
impl<A: Clone> Entry<A> {
  ...
  fn unwrap_values_mut(&mut self, pool: &RRBPool<A>) -> &mut Chunk<A> {
    match self {
      ...
      _ => {
        panic!("rrb::Entry::unwrap_values_mut: expected values, found nodes")
      }
    }
  }
...
impl<A: Clone> Clone for Node<A> {
  fn clone(&self) -> Self { Node { children: self.children.clone() } }
}
...
impl<A: Clone> Node<A> {
  pub(crate) fn new() -> Self { Node { children: Empty } }
  ...
  pub(crate) fn parent(
    ...
    Node { children: Nodes(size, PoolRef::new(&pool.node_pool, children)) }
  }

  pub(crate) fn clear_node(&mut self) { self.children = Empty; }
  ...
  pub(crate) fn single_parent(pool: &RRBPool<A>, node: Self) -> Self {
    ...
    Node { children: Nodes(size, children) }
  }
  ...
  pub(crate) fn is_single(&self) -> bool { self.children.len() == 1 }

  pub(crate) fn is_full(&self) -> bool { self.children.is_full() }

  #[allow(dead_code)] // this is only used by tests
  pub(crate) fn number_of_children(&self) -> usize { self.children.len() }
  ...
  fn is_completely_dense(&self, level: usize) -> bool {
    ...
    if let Some(expected_size) = NODE_SIZE.checked_pow(level as u32 + 1) {
      ...
      // We overflowed a usize, there's no way we can be completely dense as we
      // know the size fits in a usize.
      false
    }
  }
      ...
      let num_drained = match side {
        Side::Right => {
          if let Entry::Nodes(ref mut size, ref mut children) = self.children {
            let rightmost =
              PoolRef::make_mut(&pool.node_pool, children).last_mut().unwrap();
        ...
        Side::Left => {
          if let Entry::Nodes(ref mut size, ref mut children) = self.children {
            let leftmost =
              PoolRef::make_mut(&pool.node_pool, children).first_mut().unwrap();
  ...
  pub(crate) fn pop_chunk(
    ...
    if level == 0 {
      ...
      match replace(&mut self.children, Empty) {
        ...
        Nodes(..) => panic!("rrb::Node::pop_chunk: branch node at leaf"),
      }
    }
    else if level == 1 {
    ...
    if level == 0 {
      ...
      match drop_side {
        Left => {
          ...
          let dropped_size =
            if target_idx > 0 { size_table[target_idx - 1] } else { 0 };
            ...
            Size::Size(ref mut size) => {
              ...
              if new_size < *size {
                ...
                *size = new_size;
                ...
                unreachable!(
                  "this means node is empty, should be caught at start of \
                   method"
                );
              }
            }
    ...
    if level == 0 {
      ...
      let merged = {
        if level == 1 {
          ...
          let right_first = if let Entry::Nodes(
            ref mut size,
            ref mut children,
          ) = right.children
          {
  ...
  pub(crate) fn assert_invariants(&self, level: usize) -> usize {
    ...
    match self.children {
      ...
      Entry::Nodes(ref size, ref children) => {
        ...
        let should_be_dense =
          if let Size::Size(_) = size { true } else { false };
  ...
  // pub fn print<W>(&self, f: &mut W, indent: usize, level: usize) ->
  // Result<(), fmt::Error> where
  //     W: fmt::Write,
  //     A: fmt::Debug,
  // {
